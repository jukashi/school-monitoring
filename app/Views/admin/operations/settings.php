...
<div class="detail-grid">
    ...
    <section class="card form-section"><h2>Publish announcement</h2><form method="post" action="<?=e(url('/admin/announcements'))?>" class="stack-form" data-announcement-form><?=Csrf::field()?><label>Title<input required maxlength="180" name="title"></label><label>Message<textarea required name="body" rows="4"></textarea></label><label>Audience<select name="audience"><?php foreach(['all','students','teachers','staff'] as $x):?><option value="<?=$x?>"><?=e(ucfirst($x))?></option><?php endforeach;?></select></label><div class="grid two"><label>Publish at<input type="datetime-local" name="published_at" data-publish-at></label><label>Expires at<input type="datetime-local" name="expires_at" data-expires-at><small>Must be later than the publish time.</small></label></div><button class="button primary">Publish announcement</button></form></section>
</div>
<section class="card table-card section-card"><h2>Recent announcements</h2><table><thead><tr><th scope="col">Title</th><th scope="col">Audience</th><th scope="col">Published</th><th scope="col">Expires</th><th scope="col">Creator</th></tr></thead><tbody><?php foreach($announcements as $a):?><tr><td><strong><?=e($a['title'])?></strong><br><small><?=e(mb_strimwidth($a['body'],0,100,'…'))?></small></td><td><?=e($a['audience'])?></td><td><?=e($a['published_at']?:'Draft')?></td><td><?=e($a['expires_at']?:'No expiry')?></td><td><?=e($a['creator'])?></td></tr><?php endforeach;?><?php if(!$announcements):?><tr><td colspan="5" class="empty">No announcements yet.</td></tr><?php endif;?></tbody></table></section>
<script>
(()=>{
    const announcementForm=document.querySelector('[data-announcement-form]');
    if(announcementForm){
        const published=announcementForm.querySelector('[data-publish-at]');
        const expires=announcementForm.querySelector('[data-expires-at]');
        const syncExpiry=()=>{const now=new Date();const localNow=new Date(now.getTime()-now.getTimezoneOffset()*60000).toISOString().slice(0,16);expires.min=published.value||localNow;};
        published.addEventListener('change',syncExpiry);
        syncExpiry();
    }
