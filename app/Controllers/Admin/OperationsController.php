...
use App\Core\Auth;use App\Core\Database;use App\Core\View;use App\Services\AnnouncementService;use InvalidArgumentException;use Throwable;
final class OperationsController
{
    ...
    public function addAnnouncement():void{try{$id=(new AnnouncementService(Database::connection()))->publish((int)Auth::user()['id'],$_POST);Auth::audit('announcements.created','announcements',(string)$id);flash('success','Announcement published.');}catch(InvalidArgumentException $e){flash('error',$e->getMessage());}catch(Throwable $e){flash('error','The announcement could not be published.');}Auth::redirect('/admin/settings');}
